...
public:
  ...
  ExprPlanBetween(std::unique_ptr<ExprPlanColumn> child, Value&& lower_bound, Value&& upper_bound)
  ...
  std::unique_ptr<ExprPlan> clone() const override {
    ...
    return std::make_unique<ExprPlanBetween>(
        std::make_unique<ExprPlanColumn>(child->column), std::move(lower_bound_clone),
  ...
  DataType get_datatype() const override {
    return child->column.info.datatype;
  }
  ...
  std::ostream& print_to_ostream(std::ostream& os) const override {
    os << child->column.alias << '.' << child->column.info.name;
    os << " BETWEEN [";
    os << lower_bound;
    os << " , ";
    os << upper_bound;
