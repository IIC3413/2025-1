...
public:
  ...
  JoinPlan(
      std::vector<std::unique_ptr<LogicalPlan>>&& _children,
  ...
  std::unique_ptr<LogicalPlan> clone() const override {
    ...
    return std::make_unique<JoinPlan>(std::move(children_clone), std::move(join_columns_clone));
  }
  ...
  std::ostream& print_to_ostream(std::ostream& os, int indent = 0) const override {
    ...
    os << "Join(";
    os << join_columns[0].first.alias << "." << join_columns[0].first.info.name;
    os << " == ";
    os << join_columns[0].second.alias << "." << join_columns[0].second.info.name;
    for (size_t i = 1; i < join_columns.size(); ++i) {
      os << " AND ";
      os << join_columns[i].first.alias << "." << join_columns[i].first.info.name;
      os << " == ";
      os << join_columns[i].second.alias << "." << join_columns[i].second.info.name;
    }
    os << ")\n";
