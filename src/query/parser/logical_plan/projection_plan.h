...
public:
  ...
  ProjectionPlan(
      std::unique_ptr<LogicalPlan> child, bool distinct, int64_t limit, std::vector<Column>&& columns
  )
  ...
  std::unique_ptr<LogicalPlan> clone() const override {
    ...
    return std::make_unique<ProjectionPlan>(child->clone(), distinct, limit, std::move(columns_clone));
  }
  ...
  std::ostream& print_to_ostream(std::ostream& os, int indent = 0) const override {
    ...
    os << columns[0].alias << "." << columns[0].info.name;
    for (size_t i = 1; i < columns.size(); i++) {
      os << ", " << columns[i].alias << "." << columns[i].info.name;
    }
    if (limit != INT64_MAX) {
      os << " LIMIT " << limit;
    }
